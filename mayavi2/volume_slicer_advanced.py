...
from traits.api import HasTraits, Instance, Array, \
    Bool, Dict, List, on_trait_change
...
class VolumeSlicer(HasTraits):
    ...
    def update_data(self):
        ...
        print "trying to update position"
        self.data_src.data = self.data
    ...
    def make_side_view(self, axis_name):
        ...
        # init ellipse stack plots
        if axis_name is "z":   
            self.ellipses[axis_name] = util.plot_ellipse(15, 15, 5, 3, 3, 0, scene.mayavi_scene)
            #util.plot_ellipse(15.5, 15.5, 5, 4, 4, 0, scene.mayavi_scene)
            #self.ellipses[axis_name].mlab_source.set(x=[15], y=[15], z=[z], i=[250])
    ...
    def update_ellipse_stack(self):
        ...
        for axis_name, axis_number in self._axis_names.iteritems():
            ...
            if axis_name is 'z':
                ...
                print "z-pos", z
                e = self.ellipse_stack[z]
                dat = util.ellipse(e.cx+0.5, e.cy+0.5, e.rx, e.ry, e.alpha, n)
                dx, dy, dz, dv = dat[0], dat[1], [z]*(n+1), [25]*(n+1) 

                self.ellipses[axis_name].mlab_source.set(x=dx, y=dy, z=dz, i=dv)
    ...
    def update_position(self):
        ...
        for axis_name, axis_number in self._axis_names.iteritems():
            ...
            self.cursors[axis_name].mlab_source.set(
                                                ...
                                                y=position2d[1],
                                                z=0)
        # update ellipses as well
        self.update_ellipse_stack()
...
if __name__ == '__main__':
    ...
    ellipses = {"A": "hello kitty"}
    m = VolumeSlicer(data=data, ellipses=ellipses)
    m.configure_traits()
    m.ellipses = {"A": "hello muhahah"}
