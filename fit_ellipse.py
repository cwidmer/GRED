...
def fit_ellipse_stack(dx, dy, dz, di):
    ...
    for z in dat.keys():
        dat_layer = numpy.array(dat[z])        

        assert dat_layer.shape[1] == 2

        # fit ellipse
        try:
            [c, a, b, alpha] = fitellipse(dat_layer, opt='linear')
...
def fitggk(x):
    ...
    This exactly corresponds to what Shefali is doing fore Ellipsoids
    '''
